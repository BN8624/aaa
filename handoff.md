...
- **Q8 (실패=실행채널 문제?) 진행 중 — 현재 주 질문.** **도구 갖춰짐**: analyzer가 매 회차 verify_channel 재실행 분류(alive/reject/broken/silent/inputmismatch)+채널 자동 부착, broken vs 채널불일치 자동 분리. → FINDINGS §11.
  - ★ **부분 반례가 재현됐지만 좁아짐(§12·§14·§15)**: vtx_20·22 broken은 채널이 아니라 AST 데이터계약 불일치였으나, vtx_23~25에서는 C AST 스키마 불일치가 직접 재현 안 됨. 새 비채널 broken은 vtx_23_D2의 문법/데이터계약(`x` 변수 거부). E2 JSON broken들은 공통 대본 vs JSON stdin 포맷 불일치로 분리.
- 보류: H2(모델 통일로 비교군 소멸), H4(도커 미구현). 미착수: H3. → 정본 리스크 레지스터.

## 2. 다음 한 수 (하나만 고르고 그것만)

직전 완료(2026-06-09): ①analyzer+verify_channel 통합 ✅ ②FINDINGS §11(vtx_14~17 닫음) ✅ ③Windows cp949 버그 수정 ✅ ④FINDINGS §12(vtx_20 닫음 — broken이 stdin채널→데이터계약으로 첫 이동) ✅ ⑤회차완료 디스코드 웹훅 알림 구현/환경 우선순위 수정 ✅ ⑥vtx_21 429 사건 → client 에러분류·429복구 재설계(FINDINGS §13) ✅ ⑦vtx_22 완주(FINDINGS §14) ✅ ⑧vtx_23~25 완주(FINDINGS §15) ✅ — 30/30 유효, 429 fake 없음, H1b 0, C AST 불일치 직접 재현 없음.
...
1. **분류 갭 메우기** ★ 추천. 데이터가 호명했으니 `DATA_CONTRACT_GRAMMAR`(vtx_23_D2), `STDIN_FORMAT_MISMATCH`(JSON stdin), `STATIC_H1B_FALSE_POSITIVE`(vtx_25_D2 class 경계지만 정상)를 analyzer/verify 쪽 라벨로 분리. Q8 표가 깨끗해지고, “진짜 모델 내부 깸”과 “대본 포맷 불일치”가 자동 분리됨.
...
1. **Q8 새 회차 — vtx_26+** — 분류 갭을 메운 뒤 누적 권장. 계속 돌릴 수는 있으나, 지금은 JSON stdin/문법계약/정적 false positive가 한 라벨에 섞여 분석 비용이 커짐.
...
- `github.com/BN8624/aaa` (main). tip은 웹 Edit로 자주 바뀜 → 폰 `git fetch && git reset --hard origin/main`로 맞출 것. 직전 작업: webhook env precedence 수정, vtx_23~25 결과, FINDINGS §15, handoff 갱신(2026-06-09).
