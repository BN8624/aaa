...
직전 완료(2026-06-09): ①~⑱ ✅ ⑲관측 천장 해소(argv+DUMMY_STDIN, §24) ✅ ⑳예약 작업 Docker 권한 영구 해결(§23) ✅ ㉑h4_9 무효·h4_10 유효(§22·§25) ✅ ㉒argv 부작용 해결 — `DUMMY_ARGV=["1"]`(§26) ✅.

★ §13 수정 핵심(client.py·limiter.py·run.py·batch.py): 400/401/403/404=재시도금지, 429=재시도+백오프, 최종실패 시 배치 중단. h4_10 E2에서 정상 동작 확인.

**바로 다음: h4_11+ 누적 (argv=["1"] 검증).** 코드 변경 완료(§26), 미커밋·미푸시. 순서:
1. commit + push.
2. `/업데이트`(Discord) — pull + 예약 작업 재시작(Docker 권한 보장).
3. `/도커실행 h4_11` 또는 `/연속도커 h4_11 h4_12`.
4. 확인: C·D 표현식 파서 완주(DATA_CONTRACT_GRAMMAR 부작용 소멸), A2 argparse inputmismatch 0 유지, H1b 누적.

이후 후보: **H3/known_failures 실험 설계** — Docker 관측층 이후 자연스러운 다음 단계.
