...
def run_in_subprocess(codes: dict, entry_point: str, *, timeout: int = 20) -> dict:
    ...
    try:
        ...
        # 2) entry_point를 그 폴더 안에서 실행. stdin은 빈 파이프(EOF), timeout 둠.
        #    sys.executable로 지금 파이썬과 같은 인터프리터 사용(/usr/bin/python3).
        try:
            proc = subprocess.run(
                [sys.executable, entry_point],
                cwd=workdir,
                stdin=subprocess.DEVNULL,        # 빈 입력 → input()은 EOFError
