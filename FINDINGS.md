...
- 두 수정 모두 Task Scheduler 설정 변경(코드 무관, 원래 repo에 백업 없던 단일 실패점). 작업 재등록 시 재적용 필요: StopOnIdleEnd=false, RunOnlyIfIdle=false, RestartOnIdle=false, MultipleInstances=Parallel, LogonType Interactive(§23).
- **백업 추가**: `register_bot_task.ps1`(repo) — 위 설정 전부를 멱등 등록하는 복구 스크립트. 작업 삭제/기계 재구성 시 `powershell -ExecutionPolicy Bypass -File .\register_bot_task.ps1 -Run` 한 줄로 복구. 등록 후 박힌 값을 Get-ScheduledTask로 즉시 검증 출력. (작업 액션 = `pythonw.exe discord_bot.py`, 작업 인스턴스 수명 = 봇 수명.)
