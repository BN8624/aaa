...
The program is interactive: it reads from standard input (input() calls, usually a menu loop).
Produce a sequence of stdin lines that drives the program through its MAIN features and then
makes it EXIT cleanly.

Output RULES (critical):
- Output ONLY the raw input lines, one per line. Nothing else.
- No prose, no explanation, no numbering, no markdown, no backticks, no quotes.
- READ THE CODE to find the exact menu option values and the order of input() prompts.
  If the menu uses numbers, output the number; if it expects letters/words, output those literally.
- Exercise 1-3 main features (e.g. add an item and supply its follow-up fields, then list them),
  then choose the program's EXIT/QUIT option so it terminates. If the exit value is unclear,
  add a couple of plausible exit lines at the end.
- Keep it short: about 5 to 15 lines.
"""
...
#   [1~2] 키 있으면 실호출 1회 — 메뉴형 가짜 프로그램에 대본이 메뉴를 넘겨 quit로 끝나나 본다.
# ======================================================================
if __name__ == "__main__":
    ...
    print("\n=== 통과 ✓ === (대본이 메뉴를 넘겨 4=quit로 끝나면 길B가 먹는다)")
