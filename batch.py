...
TASKS = [
    ...
    ("A1", "A", "간단한 사칙연산 계산기 CLI. 두 수와 연산자(+,-,*,/)를 받아 결과를 출력. 0으로 나누기 처리."),
    ("A2", "A", "섭씨↔화씨 단위 변환기 CLI. 값과 방향을 받아 변환 결과를 출력."),

    # B. 멀티파일 CRUD (파일 간 인터페이스 등장)
    ("B1", "B", "할 일 목록 관리 프로그램. 추가/삭제/완료/목록 4기능. 저장은 로컬 JSON 파일."),
    ("B2", "B", "메모 관리 프로그램. 메모 추가/수정/삭제/목록 기능. 저장은 로컬 파일."),

    # C. 상태 공유 (데이터 계약이 진짜 시험되는 곳 — 5장)
    ("C1", "C", "간단한 가계부. 수입/지출 기록 추가, 잔액 계산, 내역 목록. 저장은 로컬 파일."),
    ("C2", "C", "재고 관리 프로그램. 품목 입고/출고, 현재 수량 조회, 품목 목록. 저장은 로컬 파일."),

    # D. 이벤트 체인 (한 동작이 여러 파일을 연쇄로 건드림)
    ("D1", "D", "간단한 주문 처리. 주문이 들어오면 재고를 줄이고 주문 내역에 기록. 재고 부족 시 거절."),
    ("D2", "D", "회원 포인트 시스템. 구매하면 포인트 적립, 포인트로 결제 가능, 잔여 포인트 조회."),

    # E. 의도적 실패 영역 (무너지는 걸 보려고 일부러 어렵게)
    ("E1", "E", "사용자 권한 관리. 역할(admin/user/guest)별로 접근 가능한 기능이 다르고, 권한 검사 후 동작 수행."),
    ("E2", "E", "쇼핑 장바구니. 쿠폰 적용, 재고 확인, 장바구니 합계 계산이 서로 맞물려 동작."),
]


def run_batch(*, save_dir: str = ".", runs_path: str = "runs.jsonl",
              timeout: int = 20, tag: str = None,
              stdin_input: str = DUMMY_STDIN) -> list:
